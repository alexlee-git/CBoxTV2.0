...
////            mHistoryPostion = 0;
    ...
    private OnPlayerStateChange mOnPlayerStateChange = new OnPlayerStateChange() {
        ...
        public boolean onStateChange(boolean fullScreen, int visible, boolean videoPlaying) {
            LogUtils.d("PlayerState", "fullscreen=" + fullScreen + " visible=" + visible + " " +
    ...
    public void destroy() {
        ...
        if (mNewTvAlterChange != null) {
            mNewTvAlterChange.destroy();
            mNewTvAlterChange = null;
        }
        ...
        if (view.getParent() != null && view.getParent() instanceof ViewGroup) {
            ...
            if (viewGroup.getId() == android.R.id.content) {
                if (isExit) {
                    ...
                    MarginLayoutParams layoutParams = (MarginLayoutParams) viewGroup
                            .getLayoutParams();
    ...
    public void playAlternate(String alternateId, String title, String channelId) {
        ...
        mPlayerContract.setPlayerState(PlayerContract.STATE_NORMAL);
        if (mNewTvAlterChange != null) {
    ...
    public void changeAlteranteLive(LiveInfo liveInfo, String contentId, String channel, String
        ...
        if (mChangeAlternateListener != null) {
            mChangeAlternateListener.changeAlternate(contentId, title, channel);
        }
        ...
        mPlayerContract.setPlayerState(PlayerContract.STATE_NORMAL);
        if (mNewTvAlterChange != null) {
            mNewTvAlterChange.setCurrentId(contentId);
            mNewTvAlterChange.dismiss();
        }
    ...
    private void dismissChildView() {
        ...
        mPlayerContract.setCurrentShow(SHOWING_NO_VIEW);
...
//                }
    ...
    protected void AllComplete(boolean isError, String info) {
        if (defaultConfig != null && defaultConfig.isAlternate) {
...
//            return;
    ...
    public void onVodchkResult(VideoDataStruct videoDataStruct, String contentUUID) {
        ...
        if (videoDataStruct.isTrySee() && defaultConfig != null && !defaultConfig.isAlternate) {
            isTrySee = true;
            hintVip.setVisibility(View.VISIBLE);
...
//                goToBuy();
        ...
        if (defaultConfig != null && defaultConfig.playType != PLAY_TYPE_ALTERNATE) {
            if (buyGoodsBusiness == null) {
        ...
        if (defaultConfig != null) {
            if (defaultConfig.currentAlternate != null && defaultConfig.isAlternate) {
                videoDataStruct.setCanRequestAd("1".equals(mAlternatePresenter.needRequestAd()));
            }
    ...
    public void onAlternateResult(String alternateId, List<Alternate> alternateList, int
        ...
        if (mAlternatePresenter != null && mAlternatePresenter
            ...
            defaultConfig.currentAlternate = mAlternatePresenter.getCurrentAlternate();
            if (alterTitle != null && defaultConfig != null) {
    ...
    private void uploadExitLbLog() {
        try {
            if (defaultConfig.isAlternate) {
                LogUploadUtils.uploadLog(Constant.LOG_LB, "1," + mAlternatePresenter
                        .getCurrrentChannel());
            }
        } catch (Exception e) {
        }
