...
public class BuyGoodsBusiness {
    ...
    private void showAd(AdBean.AdspacesItem item) {
        Log.e(TAG, "showAd: ");
        if (item == null || item.materials == null || item.materials.size() == 0) {
            Log.i(TAG, "数据不合法");
            return;
        }
        this.item = item;
...
//        getSkuInfo("1016913617");
    ...
    private void getResult(String authCode, long expiresIn) {
        Log.e(TAG, "getResult: " + authCode + "," + expiresIn);
            ...
            public void netDataCallback(int code, Object inParam, Object outParam) {
                if (code == 0 && outParam != null) {
                    ...
                    if (TextUtils.equals(recv.getCode(), "200")) {
                        ...
                        Log.e(TAG, "expiresIn: " + expiresIn + ",time:" + time + ",uid:" + uid + ",userNick:" + userNick + ",avatar:" + avatar);
            ...
            Observable.interval(0, 1, TimeUnit.SECONDS)
                    ...
                    .subscribe(new Observer<Long>() {
                        ...
                        public void onNext(Long aLong) {
                            if (NewTVLauncherPlayerViewManager.getInstance().isLiving() || item == null) {
                                disposable.dispose();
                                return;
                            }
