...
public class DramaFragment extends BaseFragment {
    ...
    private String contentType = "PS;CS;CG;PG";
    private TextView mDramaLabelTitle;
    private View mLabelFocusView;
    private String mKeywordType="name";
    private DramaSizeListener dramaSizeListener;
    public void setDramaSizeListener(DramaSizeListener dramaSizeListener){
        if (dramaSizeListener!=null){
            this.dramaSizeListener = dramaSizeListener;
        }
    }

    public  interface  DramaSizeListener{

        void DramaSize(int size);
    }
    public static DramaFragment newInstance(Bundle paramBundle) {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.newtv_search_result_fragment_drama, null, false);
        mRecyclerView = (SearchRecyclerView) view.findViewById(R.id.id_search_result_drama_recyclerView);
        mResultTotalView = (TextView) view.findViewById(R.id.id_fragment_drama_result_total);
        mEmptyView = (TextView) view.findViewById(R.id.id_search_result_drama_empty);
...
//                mDatas = new ArrayList<>();
    ...
    public void setDramaLabelTitle(TextView view) {
        mDramaLabelTitle = view;
    }
        ...
        try {
            if (result != null) {
                if (result.getResultList() != null && result.getResultList().size() > 0) {
                    ...
                    mLoadMore = true;
                    DramaAdapter adapter = (DramaAdapter) mRecyclerView.getAdapter();
                    if (adapter == null) {
                        mDatas = result.getResultList();
                        adapter = new DramaAdapter(mContext, mDatas);
//                adapter.setHasStableIds(true);
            ...
            NetClient.INSTANCE.getSearchResultApi().getKeywordSearchResultResponse(BuildConfig.APP_KEY, BuildConfig.CHANNEL_ID, contentType, keyword, keywordType, programType, startNum, size).subscribeOn(Schedulers.io())
                ...
                public void onNext(ResponseBody value) {
                    try {
                        ...
                        if (dramaSizeListener!=null){
                            dramaSizeListener.DramaSize(mSearchResultInfos.getResultList().size());
                        }
                        if (mLoadMore) {
    ...
    class DramaAdapter extends RecyclerView.Adapter<DramaViewHolder> {
        ...
        public DramaAdapter(Context context, List<SearchResultInfos.ResultListBean> list) {
            mContext = context;
            mDatas = list;
        }
        ...
        public DramaViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(mContext).inflate(R.layout.newtv_search_result_fragment_drama_item, parent, false);
            DramaViewHolder mDramaViewHolder = new DramaViewHolder(view);
            return mDramaViewHolder;
        }

        @Override
        public void onBindViewHolder(final DramaViewHolder holder, final int position) {
            try {
                if (mDatas == null || mDatas.size() <= 0) {
                    Log.e(TAG, "----搜索结果数据为空");
                    mEmptyView.setVisibility(View.VISIBLE);
                    ...
                    holder.mFrameLayoutResultList.setOnFocusChangeListener(new View.OnFocusChangeListener() {
                        ...
                        public void onFocusChange(View view, boolean hasFocus) {
                            ...
                            if (hasFocus) {
                                onItemGetFocus(view);
                                mDramaLabelTitle.setTextColor(Color.parseColor("#13d6f9"));
                                mFocusStatus = true;
...
//                            mContext.startActivity(intent);
                                ...
                                if (i == KeyEvent.KEYCODE_DPAD_RIGHT) {
                                    ...
                                    if (position % 6 == 0) {
                                        if (mKeyboardLastFocusView != null) {
                                            ...
                                            mDramaLabelTitle.setTextColor(Color.parseColor("#ededed"));
    ...
    class DramaViewHolder extends RecyclerView.ViewHolder {
        ...
        public DramaViewHolder(View itemView) {
            super(itemView);
            mFrameLayoutResultList = (FrameLayout) itemView.findViewById(R.id.id_frameLayout_drama_list);
            mTxtTitle = (TextView) itemView.findViewById(R.id.id_drama_title);
            mPosterTitle = (TextView) itemView.findViewById(R.id.id_drama_poster_title);
            mPosterImageView = (ImageView) itemView.findViewById(R.id.id_drama_imageView_poster);
            mFocusImageView = (ImageView) itemView.findViewById(R.id.id_drama_imageView_focus);
            //适配
