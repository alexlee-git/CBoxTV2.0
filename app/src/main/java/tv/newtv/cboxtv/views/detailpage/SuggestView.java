...
//        ShowInfoTextView("正在加载...");
    ...
    private ProgramSeriesInfo.ProgramsInfo getItem(List<ProgramSeriesInfo.ProgramsInfo> infos, int
            index) {
    ...
    private void buildListView(ProgramSeriesInfo info) {
        if (info.getData().size() > 0) {
            ...
            List<ProgramSeriesInfo.ProgramsInfo> infos = info.getData();
            for (int index = 0; index < 8; index++) {
                final View target = view.findViewWithTag("cell_008_" + (index + 1));
                ProgramSeriesInfo.ProgramsInfo itemInfo = getItem(infos, index);
...
//        ShowInfoTextView(message);
    ...
    private static class AiyaAdapter extends RecyclerView.Adapter<AiyaViewHolder> {
        ...
        private AiyaAdapter(List<ProgramSeriesInfo.ProgramsInfo> data) {
            mData = data;
        }
        ...
        private ProgramSeriesInfo.ProgramsInfo getItem(int postion) {
        ...
        public void onBindViewHolder(AiyaViewHolder holder, int position) {
            if (holder != null) {
                ProgramSeriesInfo.ProgramsInfo data = getItem(position);
                holder.setData(data);
            }
        }
    ...
    private static class AiyaViewHolder extends RecyclerView.ViewHolder {
        ...
        ProgramSeriesInfo.ProgramsInfo infoData;
    ...
    private class SuggestViewHolder {
        ProgramSeriesInfo.ProgramsInfo itemInfo;

        SuggestViewHolder(final View target, int index, ProgramSeriesInfo.ProgramsInfo info) {
            itemInfo = info;
            if (target != null) {
                if (itemInfo != null) {
