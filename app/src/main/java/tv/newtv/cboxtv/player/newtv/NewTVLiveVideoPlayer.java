...
public class NewTVLiveVideoPlayer implements ILiveVideoPlayerInterface {
    ...
    private iICntvPlayInterface mIcntvPlayerCallback = new iICntvPlayInterface() {
        ...
        public void onPrepared(LinkedHashMap<String, String> linkedHashMap) {
            ...
            //央视网日志
            isSuccessful = true;
            if (YSLogUtils.getInstance(mContext).getmLivePlay() != null && YSLogUtils.getInstance(mContext).getmLiveMetaInfo() != null) {
                // 央视网日志：结束加载
                YSLogUtils.getInstance(mContext).getmLivePlay().endPreparing(isSuccessful, YSLogUtils.getInstance(mContext).getmLiveMetaInfo());
                YSLogUtils.getInstance(mContext).getmLivePlay().onStateChanged(GSVideoState.PLAYING);
        ...
        public void onCompletion(int type) {
            ...
            if(type ==  iICntvPlayInterface.VIDEO_COMPLETE_TYPE ||
                ...
                //slp
                if (YSLogUtils.getInstance(mContext).getmLivePlay() != null && mIsLivePositiveOnPrepared) {

                    // 央视网日志：
                    YSLogUtils.getInstance(mContext).getmLivePlay().onStateChanged(GSVideoState.STOPPED); // 视频播放完成时调用
        ...
        public void onBufferStart(String s) {
            ...
            if (s.equals("VideoStartBuffer")) {
                ...
                if (mIcntvLive != null) {
                    ...
                    if (!outSourceId.equals(PlayerUrlConfig.getInstance().getPlayingContentId())) {
                        ...
                        YSLogUtils.getInstance(mContext).initCNTVVideoTracker(YSLogUtils.YS_LIVE_LOG);
                        mIsLiveInit = true;
                        // 央视网日志：视频开始加载
            ...
            if (s.equals(ON_BUFFER_START_TYPE_701_STATUS)
                ...
                if (YSLogUtils.getInstance(mContext).getmLivePlay() != null && mIsLivePositiveOnPrepared) {
                    // 央视网日志：卡顿时调用
                    YSLogUtils.getInstance(mContext).getmLivePlay().onStateChanged(GSVideoState.BUFFERING); // 卡顿时调用
                    Log.i(TAG,
        ...
        public void onBufferEnd(String s) {
            ...
            if (s.equals(ON_BUFFER_END_TYPE_702_STATUS)
                ...
                if (YSLogUtils.getInstance(mContext).getmLivePlay() != null && mIsLivePositiveOnPrepared) {
                    // 央视网日志：播放时调用
                    YSLogUtils.getInstance(mContext).getmLivePlay().onStateChanged(GSVideoState.PLAYING); // 播放时调用
                    Log.i(TAG,
