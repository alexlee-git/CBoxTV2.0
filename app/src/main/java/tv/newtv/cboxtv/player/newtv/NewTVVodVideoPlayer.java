...
//                }
                ...
                if (YSLogUtils.getInstance(mContext).getmVodPlay() != null && YSLogUtils.getInstance(mContext).getmVodMetaInfo() != null) {
                    YSLogUtils.getInstance(mContext).getmVodMetaInfo().videoDuration = mIcntvPlayer.getDuration() / 1000;
                    Log.i(TAG,
                            "---播放原始时长:"
                                    + mIcntvPlayer.getDuration());
                    Log.i(TAG, "---播放时长:"
                    ...
                    // 央视网日志：结束加载
                    YSLogUtils.getInstance(mContext).getmVodPlay().endPreparing(isSuccessful, YSLogUtils.getInstance(mContext).getmVodMetaInfo());
                    Log.i(TAG, "---结束加载开始播放正片");
                    YSLogUtils.getInstance(mContext).getmVodPlay().onStateChanged(GSVideoState.PLAYING); // 播放时调用
                    Log.i(TAG,
                            "---onPrepared:mVodPlay.onStateChanged(GSVideoState.PLAYING)");
                }
            }
            ...
            if(type ==  iICntvPlayInterface.VIDEO_COMPLETE_TYPE) {
                ...
                if (mIcntvPlayer != null) {
                    ...
                    if (YSLogUtils.getInstance(mContext).getmVodPlay() != null && mIsPositiveOnPrepared) {
                        ...
                        // 央视网日志：
                        YSLogUtils.getInstance(mContext).getmVodPlay().onStateChanged(GSVideoState.STOPPED); // 视频播放完成时调用
        ...
        public void onBufferStart(String s) {
            ...
            if (s.equals("VideoStartBuffer")) {
                ...
                if (mIcntvPlayer != null) {
                    ...
                    // 央视网日志：视频开始加载
                    if (!outSourceId.equals(PlayerUrlConfig.getInstance().getPlayingContentId())) {//判断小屏时是否已经播放，空时未播放
                        //slp
                        YSLogUtils.getInstance(mContext).initCNTVVideoTracker(YSLogUtils.YS_VOD_LOG);//初始化
...
//                }
                ...
                if (YSLogUtils.getInstance(mContext).getmVodPlay() != null && mIsPositiveOnPrepared) {
                    // 央视网日志：卡顿时调用
                    YSLogUtils.getInstance(mContext).getmVodPlay().onStateChanged(GSVideoState.BUFFERING); // 卡顿时调用
                    Log.i(TAG,
                            "----onBufferStart:mVodPlay.onStateChanged(GSVideoState.BUFFERING)");
                }
            }
...
//            }
    ...
    public boolean playVideo(Context context, FrameLayout frameLayout, iPlayCallBackEvent callBack, VideoDataStruct videoDataStruct) {
        ...
        //extend字段
        icntvPlayerInfo.setExtend(Utils.buildExtendString());
        ...
        Log.i(TAG, "setExtend: "+icntvPlayerInfo.getExtend()+",columnId:"+icntvPlayerInfo.getColumnId());
        // 央视网日志：初始化Tracker对象
...
//        }
        ...
        if (YSLogUtils.getInstance(mContext).getmVodPlay() != null && mIsPositiveOnPrepared) {
            ...
            if (PlayerUrlConfig.getInstance().isFromDetailPage()){
                YSLogUtils.getInstance(mContext).getmVodPlay().onStateChanged(GSVideoState.BUFFERING);
            }else{
            // 央视网日志：
                YSLogUtils.getInstance(mContext).getmVodPlay().onStateChanged(GSVideoState.STOPPED); // 视频播放完成时调用
                YSLogUtils.getInstance(mContext).getmVodPlay().endPlay();
...
//        }
        ...
        if (YSLogUtils.getInstance(mContext).getmVodPlay() != null) {
            // 央视网日志：
            YSLogUtils.getInstance(mContext).getmVodPlay().onStateChanged(GSVideoState.PAUSED); // 暂停时调用
            Log.i(TAG,
                    "---pauseVideo:mVodPlay.onStateChanged(GSVideoState.PAUSED)");
        }
        if (mIcntvPlayer == null)
...
//            YSLogUtils.getInstance(mContext).mIVodInfoProvider.getPosition();
            ...
            if (YSLogUtils.getInstance(mContext).getmVodPlay() != null && mIsPositiveOnPrepared) {
                // 央视网日志：快进拖拽时
                YSLogUtils.getInstance(mContext).getmVodPlay().onStateChanged(GSVideoState.SEEKING); // 拖拽时调用
                Log.i(TAG,
                        "---seekTo:mVodPlay.onStateChanged(GSVideoState.SEEKING)");
            }
            try {
